...
while ((match = libReg.exec(source))) {
  ...
  while ((match = fnReg.exec(libContent))) {
    ...
    if (!batch) {
      ...
      const call = `system.addCallTranslation(${collection}, ${version}, stringToBytes('${libName}'), '${fromSel}', ${fnData})`
...
if (batch) {
  ...
  for (let g of groups) {
    ...
    const command = `system.addCallTranslations(1, ${version}, stb('${m.libName}'), [${fromSels}], [${toSels}], ${fnData}, { gasLimit: 3e6 })`
    setup.push(command)
  }
}
